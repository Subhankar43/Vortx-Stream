  ...
  return (
    <div style={gateStyles.bg}>
      ...
      <div style={{ ...gateStyles.card, animation: shake ? 'vx-shake 0.4s ease' : 'none' }}>
        ...
        <button onClick={attempt} style={gateStyles.btn}>
          Enter Admin Panel →
        </button>
      </div>
...
const gateStyles = {
  ...
  logo: {
    ...
    fontSize: 36, letterSpacing: '0.1em',
  ...
  shieldWrap: {
    ...
    marginBottom: 28,
    animation: 'vx-pulse 2.5s ease infinite',
  },
  input: {
  ...
  btn: {
    ...
    color: '#060d1a', fontSize: 14, fontWeight: 700,
...
function AdminPanel({ onLogout }) {
  ...
  async function handleBanUser(email) {
    setBanLoading(email);
    try {
      await adminFetch('/admin/user/delete', {
        method: 'DELETE',
  ...
  async function handleTempBan(email, hours) {
    try {
      await adminFetch('/admin/user/ban', {
        method: 'POST',
  ...
  async function handleUnban(email) {
    try {
      await adminFetch('/admin/user/unban', {
        method: 'POST',
        body: JSON.stringify({ email }),
      });
  ...
  async function saveAnnouncements() {
    ...
    try {
      ...
      await adminFetch('/admin/announcements/save', {
        method: 'POST',
        body: JSON.stringify({ announcements: newList }),
      });
  ...
  async function checkServers() {
    ...
    await Promise.all(servers.map(async s => {
      try {
        await fetch(s.url, { mode: 'no-cors', signal: AbortSignal.timeout(10000) });
        results[s.id] = 'online';
      } catch {
  ...
  const tabs = [
    ...
    { id: 'movies',      label: '🎬 Top Content' },
    { id: 'servers',     label: '🌐 Servers' },
    { id: 'maintenance', label: '🔧 Maintenance' },
    { id: 'announce',    label: '📢 Announcements' },
    { id: 'homepage',    label: '🏠 Homepage Notice' },
  ];
  ...
  return (
    <div style={panelStyles.root}>
        ...
        <div style={panelStyles.overlay}>
          <div style={panelStyles.confirmBox}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setDelConfirm(null)} style={{ ...panelStyles.btnSmall, background: 'rgba(255,255,255,0.08)' }}>
                Cancel
              </button>
              <button
                onClick={() => handleBanUser(delConfirm)}
                disabled={banLoading === delConfirm}
                style={{ ...panelStyles.btnSmall, background: '#e50914' }}
              >
      ...
      <aside style={panelStyles.sidebar}>
        <div style={panelStyles.sidebarLogo}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 4px', marginBottom: 4 }}>
          ...
          <div
            ...
            style={{ width: 36, height: 20, borderRadius: 10, background: autoRefresh ? '#00c8e8' : 'rgba(255,255,255,0.1)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}
          >
        ...
        <div style={panelStyles.sidebarFooter}>
      ...
      <main style={panelStyles.main}>
        {loading ? (
          ...
          <div style={panelStyles.errorBox}>
            ...
            <div style={{ fontSize: 12, color: '#00c8e8', marginTop: 16 }}>
              Deploy the worker.js file to your Cloudflare Worker to enable admin features.
            </div>
          </div>
        ) : (
          <>
              ...
              <div>
                ...
                <div style={panelStyles.statsGrid}>
                  ...
                  <StatCard
                    ...
                    sub="Unique users in last 24h"
                    color="#7fffb0"
                  />
                  <StatCard
                    icon={<EyeIcon />}
                    label="Total Views"
                    value={stats?.totalViews ?? movies.reduce((a, m) => a + (m.views || 0), 0)}
                  ...
                  <StatCard
                    icon={<ClockIcon />}
                    label="Watch Time"
                    value={formatTime(stats?.totalWatchSeconds)}
                    sub="All users combined"
                    color="#a78bfa"
                  />
                </div>

                <SectionHeader title="Recently Active Users" sub="Last seen activity" style={{ marginTop: 36 }} />
                <div style={panelStyles.table}>
                  ...
                  {users.length === 0 ? (
                    <div style={panelStyles.empty}>No user data yet</div>
                  ) : (
                    users.slice(0, 8).map((u, i) => (
                      <div key={i} style={panelStyles.tableRow}>
                        ...
                        <span style={{ flex: 2, color: u.currentWatch ? '#7fffb0' : 'rgba(232,244,255,0.4)', fontSize: 13 }}>
                          {u.currentWatch || u.lastWatch || '—'}
                        </span>
                ...
                <SectionHeader
                  ...
                  action={
                    <button onClick={() => {
                      ...
                      a.download = 'vortxstream-users.csv';
                      a.click();
                    }} style={{ padding: '8px 16px', borderRadius: 8, background: 'rgba(0,200,232,0.1)', border: '1px solid rgba(0,200,232,0.25)', color: '#00c8e8', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                      ⬇ Export CSV
                    </button>
                  }
                />
                    ...
                    users
                      ...
                      .map((u, i) => (
                        <div key={i} style={panelStyles.tableRow}>
                          ...
                          <span style={{ flex: 2, fontSize: 13, color: u.currentWatch ? '#7fffb0' : 'rgba(232,244,255,0.35)' }}>
                            {u.currentWatch || u.lastWatch || '—'}
                          </span>
                          <span style={{ flex: 1, fontSize: 12, color: 'rgba(232,244,255,0.45)' }}>{timeAgo(u.lastSeen)}</span>
                          <span style={{ flex: 1, fontSize: 12, color: '#a78bfa' }}>{formatTime(u.watchSeconds)}</span>
                          <span style={{ flex: 1, fontSize: 12, color: 'rgba(232,244,255,0.45)' }}>
                          ...
                          <span style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                            {u.banned ? (
                              <button
                                ...
                                style={{ ...panelStyles.banBtn, background: 'rgba(0,200,232,0.12)', border: '1px solid rgba(0,200,232,0.3)', color: '#00c8e8' }}
                              >
                                ✅ Unban
                              </button>
                            ) : (
                              <select
                                onChange={e => { if (e.target.value) handleTempBan(u.email, Number(e.target.value)); e.target.value = ''; }}
                ...
                <div style={panelStyles.table}>
                  ...
                  {movies.length === 0 ? (
                    <div style={panelStyles.empty}>
                      No watch data yet. Make sure the watch tracking is added to DetailPage.jsx.
                    </div>
                  ) : (
                    movies.map((m, i) => (
                      <div key={i} style={panelStyles.tableRow}>
                        <span style={{ width: 32, color: i < 3 ? '#f59e0b' : 'rgba(232,244,255,0.3)', fontWeight: 700 }}>
                          {i + 1}
                        </span>
              ...
              <div>
                ...
                <div style={{ marginBottom: 16 }}>
                  <button
                    ...
                    style={{ padding: '10px 20px', borderRadius: 9, background: serverChecking ? 'rgba(0,200,232,0.15)' : 'linear-gradient(135deg,#00c8e8,#0ea5e9)', border: serverChecking ? '1px solid rgba(0,200,232,0.3)' : 'none', color: serverChecking ? '#00c8e8' : '#060d1a', fontSize: 13, fontWeight: 700, cursor: serverChecking ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.2s' }}
                  >
                    {serverChecking ? (
                      <>
                        <span style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid rgba(0,200,232,0.3)', borderTop: '2px solid #00c8e8', display: 'inline-block', animation: 'vx-spin 0.7s linear infinite' }} />
                        Checking…
                      </>
                    ) : '🔍 Check All Servers'}
                  </button>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
                  {['videasy', 'vidking', 'vidfast', 'rive', 'vidsrc', 'vidapi', '111movies','vidzen'].map(s => (
                    <div key={s} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${serverChecking ? 'rgba(0,200,232,0.15)' : serverStatus[s] === 'online' ? 'rgba(127,255,176,0.2)' : serverStatus[s] === 'offline' ? 'rgba(229,9,20,0.2)' : 'rgba(0,200,232,0.1)'}`, borderRadius: 12, padding: '16px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'border 0.3s' }}>
                      <span style={{ fontSize: 14, fontWeight: 600, textTransform: 'capitalize' }}>{s}</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: serverChecking ? 'rgba(0,200,232,0.4)' : serverStatus[s] === 'online' ? '#7fffb0' : serverStatus[s] === 'offline' ? '#e50914' : '#666' }}>
                        {serverChecking
                          ? <span style={{ animation: 'vx-pulse 1s ease infinite' }}>○ Checking…</span>
                          : serverStatus[s] === 'online' ? '● Online'
                          : serverStatus[s] === 'offline' ? '● Offline'
                          : '○ Unknown'}
                      </span>
                    </div>
                  ))}
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${maintenance ? 'rgba(229,9,20,0.3)' : 'rgba(0,200,232,0.1)'}`, borderRadius: 14, padding: 24 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: maintenance ? '#e50914' : '#7fffb0', fontWeight: 600 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'rgba(232,244,255,0.4)', lineHeight: 1.7 }}>
                    When enabled, all users will see a maintenance message instead of the site content. Admin panel remains accessible.
                  </div>
                </div>
              </div>
            )}
            ...
            {tab === 'announce' && (
              <div>
                ...
                <div style={panelStyles.announceBox}>
                  <div style={{ fontSize: 13, color: 'rgba(232,244,255,0.45)', marginBottom: 8 }}>
                    One announcement per line. These will appear in the login screen announcement box.
                  </div>
                  <textarea
                  ...
                  <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                    <button onClick={saveAnnouncements} disabled={saving} style={panelStyles.saveBtn}>
                      {saving ? 'Saving…' : '💾 Save Announcements'}
                    </button>
                    <button
                      onClick={() => setAnnText(announcements.join('\n'))}
                      style={{ ...panelStyles.saveBtn, background: 'rgba(255,255,255,0.06)', color: 'rgba(232,244,255,0.7)' }}
                    >
                      Reset
                    </button>
                  </div>
                  ...
                  <div style={{ marginTop: 28 }}>
                    <div style={{ fontSize: 12, color: '#00c8e8', marginBottom: 10, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                      Preview
                    </div>
            ...
            {tab === 'homepage' && (
              <div>
                <SectionHeader title="Homepage Notice Bar" sub="Single line shown in the notice banner on HomePage" />
                <div style={panelStyles.announceBox}>
                  <div style={{ fontSize: 13, color: 'rgba(232,244,255,0.45)', marginBottom: 12 }}>
                    This text appears in the green notice bar on the homepage.
                  </div>
                  <input
                    value={noticeText}
                  ...
                  <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                    <button
                      onClick={async () => {
                        setSaving(true);
                        try {
                          await adminFetch('/admin/homepage-notice/save', { method: 'POST', body: JSON.stringify({ notice: noticeText }) });
                          showToast('✅ Homepage notice saved!');
                        } catch {
                          showToast('❌ Save failed');
                        }
                        setSaving(false);
...
// ─────────────────────────────────────────────
// Section Header (supports optional action button)
// ─────────────────────────────────────────────
function SectionHeader({ title, sub, action }) {
  return (
    <div style={{ marginBottom: 20, animation: 'vx-fadein 0.3s ease', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
      <div>
        <h2 style={{ fontSize: 22, fontWeight: 700, color: '#e8f4ff', marginBottom: 4 }}>{title}</h2>
...
const panelStyles = {
  ...
  sidebar: {
    ...
    background: '#080f1e',
    borderRight: '1px solid rgba(0,200,232,0.12)',
  ...
  sidebarBtnActive: {
    ...
    boxShadow: 'inset 0 0 0 1px rgba(0,200,232,0.2)',
  },
  sidebarFooter: { display: 'flex', flexDirection: 'column', gap: 8 },
  refreshBtn: {
    padding: '9px', borderRadius: 8,
    background: 'rgba(0,200,232,0.08)',
    border: '1px solid rgba(0,200,232,0.2)',
    color: '#00c8e8', fontSize: 13,
    cursor: 'pointer', fontFamily: 'inherit',
  },
  logoutBtn: {
    padding: '9px', borderRadius: 8,
    ...
    color: '#e50914', fontSize: 13,
    cursor: 'pointer', fontFamily: 'inherit',
  },
  main: {
    flex: 1, padding: '36px 40px',
    overflowY: 'auto', maxWidth: 1100,
  },
  statsGrid: {
    display: 'grid',
  ...
  statCard: {
    ...
    animation: 'vx-fadein 0.4s ease',
  },
  statIcon: { marginBottom: 12 },
  statVal: { fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 4 },
  statLabel: { fontSize: 13, color: 'rgba(232,244,255,0.6)', fontWeight: 500 },
  statSub: { fontSize: 11, color: 'rgba(232,244,255,0.3)', marginTop: 4 },
  table: {
    background: 'rgba(255,255,255,0.03)',
    border: '1px solid rgba(0,200,232,0.1)',
    borderRadius: 14, overflow: 'hidden',
    animation: 'vx-fadein 0.3s ease',
  },
  tableHeader: {
  ...
  empty: {
    ...
    color: 'rgba(232,244,255,0.3)', fontSize: 13,
  },
  loadState: {
    display: 'flex', flexDirection: 'column',
...
export default function AdminPage() {
  ...
  function logout() {
    sessionStorage.removeItem('vx-admin-auth');
    setAuthed(false);
  }
